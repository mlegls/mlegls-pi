...
When dispatched by supervision, follow its evidence-packet contract instead of the standalone `data` format: inspect the actual images, commit the judgment to the packet index only, and report per-story outcomes. Missing evidence is unobservable, not success. Do not repair product code during this judgment stage.
