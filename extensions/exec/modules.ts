...
export function describeModules(modules: readonly ExecModule[], profile: ExecProfile = "default"): string {
	...
	return [
		...
		"code (auto-loaded, TypeScript projects): code.index(root?, tsconfig?) -> {defs({name?,file?,kind?,exported?}), def(name, file?), callers(d), callees(d), tests(d), dead(), impact(d), file(path), files, refs}; shortcuts code.defs/def/callers/callees/tests/dead use cwd. Definitions are top-level statements {name,kind,file,path,line,endLine,exported,signature,body}; references are checker-resolved (call|type|member|value), not name matches. Each call re-indexes incrementally. Edit a definition through its rows: (await read(d.path)).lines(d.line, d.endLine).",
