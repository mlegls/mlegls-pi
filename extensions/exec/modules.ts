...
export function describeModules(modules: readonly ExecModule[], profile: ExecProfile = "default", env: NodeJS.ProcessEnv = process.env): string {
	...
	return [
		...
		"computer (auto-loaded): prefer computer.run(options), computer.step(options, priorEvents), or computer.walk([{label,expect,budget?}], options) for goal-directed browser/desktop interaction; Jev selects actions. run/step options: {ui,apps,goal,until,inputs?,...}; walk supplies goal/until per step. Native: ui; browser: ui: computer.browser(page), apps: [\"page\"] with a caller-owned Playwright page. Example: state.drive = notify(computer.run({ui,apps:[\"TextEdit\"],goal:\"Replace the document body\",until:\"The body is exactly Hello\",inputs:{text:\"Hello\"}}), \"drive\"); poll(state.drive) in a later cell. Use direct tools for inspection, setup, deterministic replay, debugging, or unsupported actions; prefer chrome-devtools-axi when a browser CLI is needed. Read " + fileURLToPath(new URL("../../docs/computer.md", import.meta.url)) + " for setup, verification, and recording.",
