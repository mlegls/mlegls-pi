...
export function describeModules(modules: readonly ExecModule[], profile: ExecProfile = "default", env: NodeJS.ProcessEnv = process.env): string {
	...
	return [
		...
		"memory (auto-loaded): show(memory.recall({ids:[\"original-entry-id\"],offset?,limit?})) retrieves cited originals on this cell's branch; replaces memory_recall. Normal output filtering applies; show.raw for exact evidence. ab memory recall ID offers the same reader in bash.",
