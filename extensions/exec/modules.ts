...
export function describeModules(modules: readonly ExecModule[], profile: ExecProfile = "default", env: NodeJS.ProcessEnv = process.env): string {
	...
	return [
		...
		"await show(value, ...) renders bounded output once its values/promises resolve; values within one call keep their order, separate calls render as each resolves. Text is capped at 16 KiB per cell; show.large(value, ...) raises that cell to a 50 KiB ceiling. Omission notices count rendered UTF-8 bytes and suggest slicing/retrying; images bypass that cap, max 8 images / 20 MiB base64 per cell (visible warning; retained values stay intact).",
