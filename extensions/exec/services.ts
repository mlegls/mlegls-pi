...
const SERVICE_HELP = {
	...
	wm: {
		spawn: { signature: "spawn({run?, from?: \"fork\" | \"summary\", workers: {handle, prompt, model?, effort?, command?, agent?, base?, from?}[], wake?, wait?})", returns: "{workers, subscribed, outcomes?, pending?, aborted?}", notes: "run required until remembered by successful spawn. from=fork adds --fork <parent session file> to the Pi command; summary prepends an extract of the parent session. Per-worker from overrides the batch. Subscribes to worker reports; wake defaults true. wait=true waits for all." },
	...
	async function wm(method: string, args: unknown, signal: AbortSignal): Promise<unknown> {
		switch (method) {
			...
			case "spawn": {
				...
				const specs = (a.workers as Array<{ handle: string; prompt: string; model?: string; effort?: string; command?: string; agent?: string; base?: string; from?: "fork" | "summary" }> | undefined) ?? [];
				...
				const settled = await Promise.allSettled(specs.map(async (o) => {
					...
					const w = await spawn({ run: r, handle: o.handle, prompt: o.prompt, model: o.model, effort: o.effort, command: o.command, agent: o.agent, base: o.base, cwd, parentSession: sessionId, parentSessionFile: sessionFile, from });
