...
The checkpoint instruction opens with a reflective framing before the mechanics. It is written in a diaristic, free-indirect register ("Now a memory is about to form... What surprised me?") rather than as a command, so the model reviews its own processing instead of performing a summarization task. It states accurately that earlier positions' keys and values are reachable, tells the model whether every assistant turn in context came from the current model (`selfAuthored`), and says introspective impressions count as signal but need anchoring to cited moments. The motivation is Vogel's "Small Models Can Introspect, Too" (an accurate mechanism description plus an explicit "you can introspect" raised injected-concept detection in Qwen2.5-Coder-32B from about 0.5% to 53%, while final layers suppressed such reports) and Lindsey's "Emergent Introspective Awareness". Details record `register: diary-v1` and `selfAuthored` for later comparison; alternatives are tracked in `docs/issues/compaction-register-variants.md`.
