...
description: "Use for /show-me-your-work or work a human will review after stepping away."
...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from the harness's per-workspace session store (Claude Code: `~/.claude/projects/<slug>/`, `<slug>` the workspace path with each "/" and "." turned into "-"). Don't read other workspaces' stores; that reads unrelated private chats. Walk the log against what actually happened:
