---
name: blast-radius
description: "Use when asked what a change could break beyond its diff."
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commits per the **information-sources** skill's code anchor.
...
3. Look where grep stops. Read the source of the library you call, and check its pinned version and any local patch. Work out when things run: microtasks, unmount and teardown, the framework's scheduling model. Follow what a symbol search misses: the JSON an API returns, a DB column, a wire format, another language reading the same bytes, a feature flag, code three hops downstream.
...
6. For a big or wide change, fan out parallel agents on distinct models or tiers with the same question and merge the answers. Different priors catch different real bugs.
