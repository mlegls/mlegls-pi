...
3. Extend the existing replay at the smallest meaningful scope. Reproduce a newly noticed promise and add its check at that moment; add a scenario only for a distinct route or condition, a linked story for an independently meaningful want. An issue can close with one added assertion. First use means first use of the changed behavior, not a new end-to-end expedition.
