...
Prepared waves: `dispatch.dispatch` ([contract](../../../../../../docs/dispatch.md)). Parent owns decomposition, dependencies and concurrency; retain receipts and long waits with `notify`.

Inside Paseo (`PASEO_AGENT_ID`): use native agents, workspaces and messaging. Supervise with `paseo wait ID`, `paseo logs ID`, `paseo send ID "message"`. Final output begins `done`, `blocked`, or `needs-input`; questions go to the parent ID. A completed turn is not assignment completion. Integrate with plain Git, then archive. See [trial/setup](../../../../../../docs/paseo.md).
