...
- change loop state: `ab supervise resume <slug> <child> verify|integrate|drop|redispatch`.
  `integrate` retries accepted work; it cannot waive missing verification. For missing screenshots/state, use `verify` to send the current review request to a fresh collector. For product repairs, steer implementation and then reverify. See `~/dev/mlegls-pi/docs/verification-evidence.md`. Do not close unmet requirements merely because the report acknowledges them.
