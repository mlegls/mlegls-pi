...
// Native Cua exposes list_apps; a browser surface is computer.browser(page) over a caller-owned Playwright page.
function native_(ui: unknown) {
 if (!ui || typeof ui !== "object") throw new TypeError("computer: ui must be the native ui module or computer.browser(page) for a caller-owned Playwright page");
 return "list_apps" in ui;
}
...
export function step(options: native.Options | playwright.Options, history: readonly any[] = []) {
 return native_(options.ui) ? native.step(options as native.Options, history) : playwright.step(options as playwright.Options, history);
}
...
export function run(options: native.Options | playwright.Options) {
 return native_(options.ui) ? native.run(options as native.Options) : playwright.run(options as playwright.Options);
}
...
export function walk(steps: readonly native.Step[], options: Omit<native.Options,"goal"|"until"> | Omit<playwright.Options,"goal"|"until">) {
 return native_(options.ui) ? native.walk(steps, options as Omit<native.Options,"goal"|"until">) : playwright.walk(steps, options as Omit<playwright.Options,"goal"|"until">);
}
