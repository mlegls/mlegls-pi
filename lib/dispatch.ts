// Dispatch a ticket with optional map/autoread output in one Jev call.
//   await dispatch({ ticket, context }, threshold);
//   bun lib/dispatch.ts ticket.md threshold [context-file] >> decisions.jsonl
...
export async function dispatch(input: DispatchInput, threshold: number) {
...
if (import.meta.main) {
	...
	if (!ticket || !threshold || process.argv.length > 5) {
		console.error("usage: bun lib/dispatch.ts ticket.md threshold [context-file]");
		process.exit(2);
	}
	console.log(JSON.stringify(await dispatch({
