...
Which register and content framing should the memory checkpoint instruction use? `extensions/memory/core.ts` `induction()` currently (`register: diary-v1`) opens with a diaristic, free-indirect framing before the mechanics:
...
- including vgel's info prompt or the Lindsey abstract nearly verbatim, versus the one-sentence evidence claim
