...
decisions:
- 2026-09-24: upstream gets only the code patch: the marker check in the `tool_result` handler, a test, and the README section documenting the marker. The system-prompt change that named exec and `loadSkill` is dropped; exec's own tool guidance (`extensions/exec/modules.ts`) already says raw SKILL.md reads need `loadSkill(path)`. Rebased onto v1.3.6 as `explicit-skill-ownership-v2` (5979e33); upstream master already fails 10 pi-docs tests locally, the same set as before the change.
