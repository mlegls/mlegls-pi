...
- collapse toggles and pins write the `.base` on every click/drag; move to plugin `saveData` if that proves noisy.
- graph has no pan/zoom; Bases filters are the way to cut it down. Obsidian's `cls` option throws on space-separated classes; pass an array.
