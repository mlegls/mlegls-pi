...
done: the function exists with a test against jev, the fallbacks work, and "[[projects/mlegls-pi/issues/archive/dispatch-script]]" can call it. jev access is already granted.

holes:
- the ask fallback mid-script: does the script suspend across the parent's turn (the async wake/block primitive), or does it fail and re-run? decide with "[[projects/mlegls-pi/issues/archive/supervision-join-script]]".
- how conversation tail is serialized as jev `state` for in-session calls; the same serialization serves "[[projects/mlegls-pi/issues/archive/ingress-filter]]".
...
- 2026-09-18: the ask fallback mid-script neither suspends nor re-runs; the script runs in a forked session and blocks on `needs-input`: "[[projects/mlegls-pi/issues/archive/context-handoff]]".
