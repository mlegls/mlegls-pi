...
- 2026-09-18: a procedure that needs the model mid-run forks the session as a worker rather than yielding control back: "[[projects/mlegls-pi/issues/archive/context-handoff]]". mechanical steps become `lib/` functions either way; a script owns control flow only when the flow between asks is worth having in code.
- 2026-09-18: the index lives in this repo; `repo-merge` moves the rest here.

- 2026-09-18: skills stay skills. `/skill:name` is the loader and the skill list is the index; the "one index" idea is dropped. a procedure migrates one at a time by moving steps into `lib/` and shrinking its SKILL.md toward the call; a half-migrated body (part call, part prose) is fine, and steps that are judgment stay prose. `dispatch` first ("[[projects/mlegls-pi/issues/archive/dispatch-script]]"). conventions and lenses (grilling, what-else, devils-advocate, variety, show-me, wizard) are unchanged. third-party: workmux and humanlayer stay; pstack (untouched since Sep 7) → `disabled/`. what scripts need that prompts don't: "[[projects/mlegls-pi/issues/archive/exec-project-modules]]" and "[[projects/mlegls-pi/issues/archive/context-handoff]]".
