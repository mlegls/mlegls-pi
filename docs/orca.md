...
Load the installed contract with `orca skills get orchestration`. The TypeScript wrapper retains native receipts; Pi launch receipts additionally report local startup evidence. For fresh assignments, use `route.prepare` to select stance, model, and effort, then [`dispatch.dispatch`](dispatch.md) to launch the prepared work. The launcher handles the Pi bootstrap-and-enrollment sequence internally; it is not a reason to choose a model manually.
...
`workers.submit({spec, model, effort, …})` creates a Task, starts a caller-owned shell bootstrap, and enrolls its terminal through native `dispatch --return-preamble` without `--inject`. The returned preamble already includes the Task. The launcher atomically publishes it plus a correlation marker to a private prompt file; the bootstrap then executes `pi --model … --thinking … @prompt.md`. Pi receives its assignment as a startup argument, not editor input. Existing Task IDs use the same path through `startPi({task, …})`.

The bootstrap waits up to `timeoutMs` (default 60000) for publication, then exits if no prompt arrives. After publication the caller waits up to `startWaitMs` (default `timeoutMs`) for the Pi extension’s correlated `before_agent_start` event. Success returns local `state: ready`, `stage: turn_started`, and `startConfirmation.status: started` alongside the native dispatch response and `clientTerminal`. This proves entry into the assigned turn, not a successful model response or settlement. Native dispatch is **unsupervised**: Task/Dispatch identity and mail remain native, but Orca does not own the process.

Missing turn-start evidence throws `OrcaError` with the retained Task/Dispatch/terminal identities and file paths in `error.receipt`. Recheck with `await orca.workers.confirmStart(error.receipt, 5000)`; this updates confirmation without resending work or consuming mail. A failed wave stops at `failed`, leaves later assignments `pending`, and still consumes capacity until the retained attempt is resolved. Do not replay the launcher to recover an uncertain mutation. Prompt, bootstrap, and evidence files remain in a private local temporary directory for inspection; this path requires a same-host POSIX terminal and shared filesystem.

Native `workers.start` remains available and conservatively returns unconfirmed. Native `terminal send --wait-submit` is not a standalone observation call: it requires text and Enter. Never repeat accepted input merely to observe it.
...
Orca 1.4.206 accepts `worker-start --agent pi`, but rejects Pi launch-time model/effort selection. `orca.startPi({spec, run?, from?, worktree?, model, effort, taskTitle?})` enrolls a bootstrap terminal, then starts Pi with native model/effort flags and a prompt-file argument. It requires an exact existing workspace selector; create a workspace first for separate checkouts. Worktrees separate Git state, not filesystem permissions. The receipt retains the native dispatch response plus local start confirmation and `clientTerminal`; errors retain any created resources for inspection.

That terminal is **caller-owned**. Orca’s `workers.release(dispatchId)` will not close a pre-existing terminal. After accepted settlement, [`dispatch.integrate`](dispatch.md#integration) merges, releases, closes the worktree's terminals and removes the worktree in one call; doing it by hand means inspecting the release receipt, then explicitly `orca.stop(receipt.clientTerminal.handle)` if that terminal has no new owner, then removing the isolated worktree after merging. Direct native `workers.start({agent: "pi", …})` avoids this ownership gap but uses Pi’s defaults; it is not a substitute for executing the routed model/effort selection.
