...
- Native release of the enrolled terminal returned `retained / external_terminal / processAction: none`. The caller explicitly stopped its PTY after settlement. This ownership difference is exposed by `clientTerminal` and recorded as [[projects/mlegls-pi/issues/archive/orca-pi-launch-ownership]].
